...
pub fn load_struct_from_json<T: DeserializeOwned>(path: &str) -> Result<T, StorageErrors> {
    let file = File::open(path).map_err(|_| StorageErrors::FsError)?;
    let mut reader = BufReader::new(file);

    let mut json_string = String::new();
    reader
        .read_to_string(&mut json_string)
        .map_err(|_| StorageErrors::JSONError)?;

    let result: T = serde_json::from_str(&json_string).map_err(|_| StorageErrors::CastError)?;
    Ok(result)
}
